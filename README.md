...
## switch-env.py — 模拟器 ↔ 实车一键切换

切换 argentina-app 叉臂 UDP 的 `controller_ip`（model.xml 请手动管理）。

```bash
# 双击 .bat 文件（最方便）
switch-real.bat                 # 切换至实车
switch-sim.bat                  # 切换至模拟器

# 或命令行
python switch-env.py --real     # 切换至实车
python switch-env.py --sim       # 切换至模拟器
python switch-env.py --status    # 查看当前模式
```

**修改的文件：** `../projects/argentina-app/app/fork_udp.py` 中的 `controller_ip`

**切换后需手动操作：** 重启 argentina-app + 启停模拟器。
